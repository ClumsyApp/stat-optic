...
class Scope(object):
    ...
    __slots__ = [
        ...
        'xaxis', 'yaxis', 'zaxis', 'graph_type', 'xaxis_label', 'yaxis_label', 'zaxis_label'
    ]
'''
    ...
    def _compute_repo(self):
        ...
        if repos:
            ...
            if isinstance(repos, str):
                if repos in [ "ALL", "None" ]:
                    if self.org:
                        self.repos = Repository.objects.filter(organization=self.org)
                    else:
    ...
    def _compute_axes(self):
        try:
            x = self.request.GET.get('xaxis', None)
            self.xaxis_label = x
            if x and x != 'Time':
                self.xaxis = Statistic.STATISTIC_FIELDS[x]
            elif x and x == 'Time':
                self.xaxis = x
            y = self.request.GET.get('yaxis', None)
            self.yaxis_label = y
            if y:
                self.yaxis = Statistic.STATISTIC_FIELDS[y]
            #self.zaxis = None
            z = self.request.GET.get('zaxis', None)
            self.zaxis_label = z
            if z != None and z != 'null' and z != '':
                self.zaxis = Statistic.STATISTIC_FIELDS[z]
            else:
                self.zaxis = None
            self.graph_type = self.request.GET.get('graph_type', None)
        except KeyError:
            #Prevents page from completely crashing
            self.xaxis = None
            self.yaxis = None
            self.zaxis = None
            self.xaxis_label = None
            self.yaxis_label = None
            self.zaxis_label = None
